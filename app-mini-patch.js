// v6-mini-patch: settings safe-call + null-guards (ONLY these changes)
(() => {
  // ---- Tabs (unchanged except safe settings call) ----
  ...
  tabs.forEach(btn => btn.addEventListener('click', () => {
    ...
    // SAFE call (no optional chaining; no crash if undefined)
  ...
  // ---- Settings listeners with null-guards (ONLY change here) ----
  ...
  const _reset = $('resetAll'); if (_reset) _reset.addEventListener('click', () => {
    ...
    save(state);
    renderHeader(); renderHome(); renderOverview();
    alert('초기화 완료');
  });
